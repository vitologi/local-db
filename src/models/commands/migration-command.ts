import {IDbMigration} from "../../interfaces/db-migration";
...
export class MigrationCommand {
  ...
  private readonly up: (db: IDb) => Promise<void>;
  ...
  constructor(config: IDbMigration, version: number, db: IDb) {
    this.db = db;
  ...
  async execute(/* type: 'up' | 'down' = 'up' */): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      const openRequest = indexedDB.open(db.name, targetVersion);
      ...
      openRequest.onupgradeneeded = (event: IDBVersionChangeEvent) => {
        ...
        this.up(db)
          .then(() => {
            console.log(`Db:${db.name} Version:${targetVersion} Status: migration base completed`);
            if (isSuccessFired) {
          ...
          .catch((err) => {
            console.error(`Db:${db.name} Version:${targetVersion} Status: migration base error`, err);
            if (transaction)
              transaction.abort();
